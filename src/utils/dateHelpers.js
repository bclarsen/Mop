...
// isTaskDone: checks if a task is currently completed / up to date
...
export const MIN_REMINDER_ADVANCE_MS = 5 * 60 * 1000;

export const COMPLETED_WINDOW_OPTIONS = [
...
export function isRecentlyCompleted(task, windowMs = DEFAULT_COMPLETED_WINDOW_MS) {
  ...
  const completedTime =
    typeof task.lastCompleted === 'string'
      ...
      : Number(task.lastCompleted);
  if (!completedTime || Number.isNaN(completedTime)) return false;
  return Date.now() - completedTime < Number(windowMs);
}

// A stored window is a single millisecond number; the Preferences UI edits it as
// days/hours/minutes/seconds. These helpers convert between the shapes.
export function msToParts(ms) {
  ...
  return {
    ...
    totalSecondsRemaining: totalSeconds % 3600,
    totalSeconds,
  };
}

export function partsToMs({ days = 0, hours = 0, minutes = 0, seconds = 0 } = {}) {
...
export function customDurationToMs(hours = 0, seconds = 0) {
  const h = Math.max(0, Number(hours) || 0);
  const s = Math.max(0, Number(seconds) || 0);
  return (h * 3600 + s) * 1000;
}
